...
HDC hdcWindow = NULL;
HDC hdcMemDC = NULL;
COLORREF memkeptarolo[SCREEN_WIDTH * SCREEN_HEIGHT];
...
LRESULT CALLBACK WndProc0(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case MM_JOY1MOVE:
		...
		if (yPos == 65535) {
			rot_degree_x2 += 5.0; GDI_drawing();
		}
		else if (yPos == 0) {
			rot_degree_x2 -= 5.0; GDI_drawing();
		}
		if (fwButtons == 128) {
			rot_degree_z2 += 5.0; GDI_drawing();
		}
		else if (fwButtons == 64) {
			rot_degree_z2 -= 5.0; GDI_drawing();
		}
		if (rot_degree_y2 > 360) {
			rot_degree_y2 = 0; GDI_drawing();
		}
		else if (rot_degree_y2 < 0) {
			rot_degree_y2 = 358; GDI_drawing();
		}
		if (rot_degree_x2 > 359) {
			rot_degree_x2 = 0; GDI_drawing();
		}
		else if (rot_degree_x2 < 0) {
			rot_degree_x2 = 358; GDI_drawing();
		}
		if (rot_degree_z2 > 359) {
			rot_degree_z2 = 0; GDI_drawing();
		}
		else if (rot_degree_z2 < 0) {
			rot_degree_z2 = 358; GDI_drawing();
		}
...
void create_main_buffer(void)
{
	...
	for (i = 0; i < SCREEN_HEIGHT; ++i)
		for (j = 0; j < SCREEN_HEIGHT; ++j)
			memkeptarolo[i * SCREEN_HEIGHT + j] = RGB(200, 200, 0);
}

void CUDA_cleanup_main_buffer(void)
{
	cudaMemset(dev_image_data, 50, SCREEN_HEIGHT * SCREEN_WIDTH * sizeof(unsigned int));
}
...
void drawing(void)
{
	...
	CUDA_CleanUp_Zbuffer << < ((SCREEN_WIDTH * SCREEN_HEIGHT) + BLOCKSIZE - 1) / BLOCKSIZE, BLOCKSIZE >> > (dev_zbuffer);
	cudaDeviceSynchronize();

	render_objects << <128, BLOCKSIZE >> > (raw_vertices_length, dev_rotated_verticesX, dev_rotated_verticesY, dev_rotated_verticesZ, dev_image_data, dev_zbuffer, vLight);
	cudaDeviceSynchronize();

	cudaMemcpy(memkeptarolo, dev_image_data, SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(unsigned int), cudaMemcpyDeviceToHost);
	swap_main_buffer();
}
...
__global__ void render_objects(int maxitemcount, float* rotarrayX, float* rotarrayY, float* rotarrayZ, unsigned int* puffer, float* zpuffer, VEKTOR fenyvektor)
{
	...
	for (i = index; i < maxitemcount - 2; i += stride)
	{
		...
		//tesztcolor = RGB(180 * ((float)i / (float)maxitemcount * 100), 180 * ((float)i / (float)maxitemcount * 100), 180 * ((float)i / (float)maxitemcount * 100));
		tesztcolor = RGB(255 * Light_intensity, 255 * Light_intensity, 255 * Light_intensity);

		//CUDA_SetPixel(rotarrayX[i], rotarrayY[i], tesztcolor,puffer);
		//CUDA_SetPixel_Zbuffer(rotarrayX[i], rotarrayY[i], rotarrayZ[i], tesztcolor, puffer, zpuffer);
		...
		/*CUDA_DrawLine_Zbuffer(rotarrayX[i], rotarrayY[i], rotarrayZ[i], rotarrayX[i + 1], rotarrayY[i + 1], rotarrayZ[i+1], tesztcolor, puffer,zpuffer);
		CUDA_DrawLine_Zbuffer(rotarrayX[i + 2], rotarrayY[i + 2], rotarrayZ[i+2], rotarrayX[i + 1], rotarrayY[i + 1], rotarrayZ[i+1], tesztcolor, puffer, zpuffer);
		CUDA_DrawLine_Zbuffer(rotarrayX[i], rotarrayY[i], rotarrayZ[i], rotarrayX[i + 2], rotarrayY[i + 2], rotarrayZ[i+2], tesztcolor, puffer, zpuffer);//*/
